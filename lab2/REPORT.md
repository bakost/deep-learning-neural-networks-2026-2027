...
[results/summary.json](results/summary.json). PDF-версия отчёта собирается
командой `python tools/build_pdf.py`.

## Кратко о результатах

1. **Ортогональность.** Генерируемый ансамбль $H = A + A^T$ с элементами
   $\mathcal N(0, 1)$ — гауссов *ортогональный* ансамбль: распределение
   $Q^T H Q$ совпадает с распределением $H$ для любой ортогональной $Q$.
   Это доказано в разделе 2 и проверено численно, в том числе на случайных
   ортогональных матрицах, построенных процессом Грама–Шмидта.
2. **Гауссов ансамбль, 2×2.** Догадка Вигнера для 2×2 не приближение, а
   точный закон. Даже при $M = 10^6$ матриц гистограмма не отличима от (15):
   $D_{KS} = 0.0006$, $p = 0.63$; критерий $\chi^2$: 37 при 38 степенях
   свободы.
3. **4×4 и 16×16.** Кривая (15) очень близка к гистограмме: максимальное
   ...
   *систематическое*: дисперсия нормированного расстояния равна 0.2824 (4×4)
   ...
   $M = 10^6$.
4. **Около нуля** (пункт 6) плотность растёт линейно, $\rho(s) \approx \frac{\pi}{2}s$.
...
5. **Матрицы из ±1, 2×2 и 4×4** (пункт 9). Распределение дискретное
...
6. **Аналитика для 2×2 из ±1** (пункт 10) выведена для обеих трактовок
...
$A$ симметризуется. Почему полученный ансамбль ортогональный, разобрано в
разделе 2.

| Ансамбль | Как строится $H$ | Элементы $H$ | Ортогональный | Где |
|---|---|---|---|---|
| `goe` | $A_{ij} \sim \mathcal N(0,1)$, $H = A + A^T$ | диагональ $\mathcal N(0, 4)$, вне диагонали $\mathcal N(0, 2)$ | **да** | пункты 1–8 |
| `pm1` | $H_{ij} = H_{ji} = \pm 1$ при $i \le j$ | $\pm 1$ | нет | пункт 9, трактовка (а) |
| `pm1-sum` | $A_{ij} = \pm 1$, $H = A + A^T$ | диагональ $\pm 2$, вне диагонали $0, \pm 2$ | нет | пункт 9, трактовка (б) |
| `normal-mirror` | $H_{ij} = H_{ji} \sim \mathcal N(0,1)$ при $i \le j$ | все $\mathcal N(0, 1)$ | нет | для сравнения, раздел 7 |
...
## 2. Ортогональность ансамбля

По требованию к генерации ансамбль должен быть *ортогональным*. Так и
определяется гауссов ортогональный ансамбль (GOE): это вещественные
симметричные матрицы $H$, распределение которых **не меняется при любом
ортогональном преобразовании**

$$
H \;\to\; H' = Q^T H Q, \qquad Q^T Q = Q Q^T = I,
$$

где $Q$ — ортогональная матрица в смысле лекции 1 (её столбцы образуют
ортонормированную систему). Отсюда и название ансамбля. Сами матрицы $H$
при этом симметричные, а не ортогональные: только так их собственные числа
вещественны и их можно упорядочить по возрастанию, как требует пункт 2.

### 2.1. Почему $H = A + A^T$ даёт ортогональный ансамбль

У $H = A + A^T$ элементы $H_{ij}$ при $i \le j$ независимы: на диагонали
$\mathcal N(0, 4)$, вне диагонали $\mathcal N(0, 2)$ (раздел 1.3
...
След не меняется при повороте:
$\operatorname{tr}\,(Q^T H Q)^2 = \operatorname{tr}\,(Q^T H^2 Q) = \operatorname{tr}\, H^2$.
Отображение $H \to Q^T H Q$ линейно и сохраняет норму Фробениуса, то есть
является поворотом пространства симметричных матриц и не меняет объём.
Значит, $H' = Q^T H Q$ имеет ту же плотность, что и $H$: **ансамбль
ортогональный**.

Ключевую роль играет соотношение дисперсий диагонали и остальных элементов
$\sigma_d^2 : \sigma_o^2 = 2 : 1$, которое даёт симметризация $A + A^T$.
Только при нём сумма в показателе складывается в $\operatorname{tr}\, H^2$.
Если взять все элементы с одинаковой дисперсией (ансамбль
`normal-mirror`), инвариантности не будет. Более того, если элементы
независимы, ансамбль может быть ортогонально инвариантным, только когда
они гауссовы с этим соотношением дисперсий (теорема из главы 2 книги Меты
[2]). Поэтому матрицы из ±1 (пункт 9) ортогональный ансамбль не образуют
ни при какой симметризации.

### 2.2. Численная проверка

Случайная ортогональная $Q$ строится как в лекции 1: процесс
Грама–Шмидта (QR-разложение) применяется к гауссовой матрице. Затем знаки
столбцов согласуются с диагональю $R$, чтобы $Q$ была распределена
равномерно по всем ортогональным матрицам (мера Хаара). Одна и та же $Q$
применяется ко всем матрицам ансамбля, и распределения элементов до и
после поворота сравниваются.

![Рисунок 1](figures/orthogonal_invariance.png)

**Рисунок 1.** (а) Матрицы 2×2, поворот на угол $\varphi$: дисперсия
$H'_{11}$ относительно исходной. Точки — моделирование, линии — формула
$\operatorname{Var} H'_{11} = \sigma_d^2\bigl(1 - \tfrac12\sin^2 2\varphi\bigr) + \sigma_o^2 \sin^2 2\varphi$.
(б) GOE: распределение $H_{11}$ до и после поворота на 30°. (в) То же для
$A + A^T$ с $A$ из ±1. (г) Матрицы 16×16 и случайная $Q$ по мере Хаара.

| Ансамбль, $n = 16$ | Var диагонали: до → после | Var вне диагонали: до → после | $H_{11}$ до и после (KS) | $H_{12}$ до и после (KS) |
|---|---|---|---|---|
| **GOE**: $\mathcal N(0,1)$, $A + A^T$ | 3.994 → 4.000 | 2.000 → 1.999 | $p = 0.40$ | $p = 0.87$ |
| ±1, $A + A^T$ | 4.000 → 3.999 | 2.000 → 2.000 | $p < 10^{-10}$ | $p < 10^{-10}$ |
| $\mathcal N(0,1)$, симметричная | 0.999 → 1.838 | 1.000 → 0.944 | $p < 10^{-10}$ | $p = 0.02$ |
| ±1, симметричная | 1.000 → 1.821 | 1.000 → 0.945 | $p < 10^{-10}$ | $p < 10^{-10}$ |

$M = 10^5$ матриц. Для сравнения распределений до и после поворота
использован двухвыборочный критерий Колмогорова–Смирнова на независимых
половинах ансамбля. Для $n = 4$ картина та же (таблица в
[results/tables.md](results/tables.md)). Для всех $Q$ отклонение
$\lVert Q^T Q - I \rVert$ не больше $9 \cdot 10^{-16}$. Спектры $H$ и
$Q^T H Q$ совпадают с точностью $3 \cdot 10^{-14}$, поэтому поворот не
меняет и расстояния между уровнями.

**Итог.**

- **GOE, то есть $A + A^T$ с нормальными элементами, — ортогональный
  ансамбль.** Дисперсии и сами распределения элементов после поворота не
  меняются: при любом угле на рисунке 1(а) и при случайной $Q$ на
  рисунке 1(г). Этот ансамбль используется в пунктах 1–8.
- **Матрицы с одинаковой дисперсией всех элементов** — не ортогональный
  ансамбль: после поворота дисперсия диагонали растёт до 1.5 (2×2, угол
  45°) и до 1.8 (16×16).
- **Матрицы из ±1 с $A + A^T$** сохраняют дисперсии (соотношение то же,
  2 : 1), но не распределение: после поворота элементы перестают быть
  равны $0, \pm 2$ (рисунок 1(в)). Ансамбли пункта 9 ортогональными быть
  не могут в принципе. Раздел 5 показывает, что при $n = 16$ это уже не
  влияет на статистику уровней.

---

## 3. Проверка теоретической части на матрицах 2×2

![Рисунок 2](figures/geometry_2x2.png)

**Рисунок 2.** (а) Точки $(d, b)$ для 4000 матриц 2×2 гауссова ансамбля и
...
Рисунок 2(а) показывает, откуда берётся отталкивание. Плотность точек
...
## 4. Гауссов ортогональный ансамбль (пункты 1–8)

### 4.1. Гистограммы и график (15) — пункты 5 и 7

![Рисунок 3](figures/goe_histograms.png)

**Рисунок 3.** Гистограммы нормированных расстояний и догадка Вигнера (15).
...
### 4.2. Поведение около нуля — пункт 6

![Рисунок 4](figures/small_s.png)

**Рисунок 4.** (а) Начало гистограмм крупным планом, с погрешностями.
...
1. **Подсчёт условий.** Чтобы два собственных числа матрицы 2×2 совпали,
   ...
   рисунок 2(а)). Это теорема фон Неймана–Вигнера о непересечении уровней:
...
### 4.3. Насколько график близок к гистограмме — пункт 8
...
![Рисунок 5](figures/residuals.png)

**Рисунок 5.** Разность «гистограмма минус (15)» для $M = 10^6$ с
...
- **Для 2×2 кривая совпадает с гистограммой в пределах статистической
  погрешности при любом $M$.** На рисунке 5 точки разбросаны около нуля
  ($\chi^2/N_{bin} = 0.8$). На рисунке 6 $D_{KS}$ убывает как
  $0.67/\sqrt M$ вплоть до $M = 10^6$. Так и должно быть: для 2×2
  формула (15) выведена точно.
...
- **Расхождение — свойство формулы (15), а не погрешность расчёта.**
  Точки для 16×16 на рисунке 5 ложатся на зелёную кривую
  «GOE$_\infty$ − Вигнер», а $D_{KS}$ на рисунке 6 выходит на полку
...
- **При минимальном ансамбле ($M = 1000$) отличие не обнаружить.**
  ...
  $M \gtrsim 10^4$, когда шум опускается ниже 0.0066 (рисунок 6).

![Рисунок 6](figures/ks_vs_size.png)

**Рисунок 6.** Как $D_{KS}$ до догадки Вигнера зависит от числа матриц
...
### 4.4. Почему пара уровней фиксирована (сноски 2 и 4)

![Рисунок 7](figures/pair_choice.png)

**Рисунок 7.** GOE 16×16, $M = 10^5$. (а) Плотность уровней и закон
...
- Если же смешать все пары и нормировать на *общее* среднее, получится
  смесь законов с разными масштабами: дисперсия 0.334, $D_{KS} = 0.025$
  до (15) (оранжевая линия на панели (г)). Если же сначала нормировать каждую
...
## 5. Матрицы из ±1 (пункт 9)

![Рисунок 8](figures/pm1.png)

**Рисунок 8.** Трактовка (а): симметричная матрица из ±1. Оранжевый контур —
...
![Рисунок 9](figures/pm1_sum.png)

**Рисунок 9.** Трактовка (б): $H = A + A^T$, $A$ из ±1. Полный перебор:
16 и 65 536 матриц.
...
пиков совпадают с точным законом из перебора (рисунки 8–9, раздел 6).

**Поведение около нуля.** Рассуждение о «двух условиях» из раздела 4.2
...
## 6. Аналитические результаты для 2×2 из ±1 (пункт 10)
...
изотропная нормальная точка на плоскости, и $s$ имеет распределение
Рэлея (13). В случае ±1 точка $(d, b)$ может занимать лишь несколько узлов
...
## 7. Дополнительно: зачем складывать $A + A^T$
...
![Рисунок 10](figures/normal_mirror.png)

**Рисунок 10.** (а) Отклонение от (15) для 2×2: GOE (синие точки) и
...
точна при $n = 2$. Для 16×16 разница исчезает (рисунок 10(в),
...
1. Ансамбль $H = A + A^T$ с нормальными элементами ортогональный: его
   плотность $\propto \exp(-\operatorname{tr}\, H^2/8)$ не меняется при
   $H \to Q^T H Q$. Проверено поворотами на произвольный угол и случайными
   ортогональными $Q$ по мере Хаара. Необходимое условие — соотношение
   дисперсий диагонали и остальных элементов 2 : 1, которое и даёт
   симметризация $A + A^T$.
2. Для гауссова ортогонального ансамбля расстояние между соседними
   ...
   миллионе матриц.
3. Для $n \ge 4$ догадка Вигнера — приближение с погрешностью около 2 % по
   ...
   а не с (15).
4. Около нуля плотность линейна, $\rho(s) \approx \frac{\pi}{2}s$
   ...
   собственных чисел.
5. Для матриц из ±1 малого размера распределение дискретное, и
   ...
   элементов.
6. Методически важно (а) фиксировать пару уровней — иначе смешиваются
   законы с разным масштабом, и отклонение от (15) вырастает в 3.5 раза;
   (б) симметризовать именно $A + A^T$ — иначе ансамбль перестаёт быть
   ортогональным, и для 2×2 формула (15) перестаёт быть точной.
